...
bool iguais(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2)
{
    ...
    if(arv1 == NULL && arv2 == NULL)
    {
        ...
        if(arv1->item.chave == arv2->item.chave)
        {
            if((arv1->esq != NULL && arv2->esq == NULL) || (arv1->esq == NULL && arv2->esq != NULL))
            {
                //printf("test2\n");
                return false;
            }
           
            if((arv1->dir != NULL && arv2->dir == NULL) || (arv1->dir == NULL && arv2->dir != NULL))
            {
                //printf("test3\n");
                return false;
            } 
                ...
                //printf("test4 - left\n");
                bool v1 = iguais(arv1->esq, arv2->esq);

                if(v1 == false) return false;

                //printf("teste4 - right\n");
                bool v2 = iguais(arv1->dir, arv2->dir);
...
int main()
{
   ...
   inicializar(arv1);
   inicializar(arv2);
   saoIguais(arv1, arv2); // iguais 1

   ITEM item;
   item.chave = 1;
   criarNo(item, &arv1); // cria o no Raiz
   saoIguais(arv1, arv2); // diferentes 2
   criarNo(item, &arv2); // cria o no Raiz   
   saoIguais(arv1, arv2); // iguais 3

   item.chave = 2;
   adicionarFilho(item, NoEsquerdo, arv1);
   saoIguais(arv1, arv2); // diferentes 4 
   adicionarFilho(item, NoEsquerdo, arv2);
   saoIguais(arv1, arv2); // iguais 5

   item.chave = 5;
   adicionarFilho(item, NoDireito, arv1);
   saoIguais(arv1, arv2); // diferentes 6 
   adicionarFilho(item, NoDireito, arv2);
   saoIguais(arv1, arv2); // iguais 7
   /*
                 1
              2     5
   */
   ...
   saoIguais(arv1, arv2); // diferentes 8
